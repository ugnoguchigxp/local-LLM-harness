# test_qwopus_dual_proxy.ps1
# Test both deployed Qwopus proxy endpoints to verify they are working and responsive.
...
$body = @{
    model = "local-model"
    messages = @(
        @{ role = "user"; content = "Hello! Please reply in one short sentence starting with 'Instance Test: ...'." }
    )
    stream = $false
    max_tokens = 50
...
foreach ($port in $ports) {
    ...
    Write-Host "`n[*] Testing Instance on Port $port..." -ForegroundColor Cyan
    try {
        $headers = @{
