...
現行の NSSM / PowerShell 運用（`start_servers.ps1`、`proxy.js`）は、Gateway が置き換えるまでの **現行 Data Plane** として残す。制御 API（`/state` `/prepare` 等）は副経路である。Agent から見た主契約は OpenAI 互換 Gateway とする。
